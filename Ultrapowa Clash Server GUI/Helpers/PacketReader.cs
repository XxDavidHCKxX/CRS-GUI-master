namespace CRS.Helpers
{
    ...
    public class PacketReader : BinaryReader
    {
        ...
        public override int Read(byte[] buffer, int offset, int count) => this.BaseStream.Read(buffer, 0, count);
        ...
        public byte[] ReadByteArray()
        {
            ...
            if (length < -1 || length > this.BaseStream.Length - this.BaseStream.Position) return null;
            var buffer = this.ReadBytesWithEndian(length, false);
            return buffer;
        }
        ...
        {
            var packetLengthBuffer = this.ReadBytesWithEndian(3, false);
        ...
        {
            var buffer = this.ReadBytesWithEndian(2);
            return BitConverter.ToUInt16(buffer, 0);
        }

        public uint ReadUInt24() => (uint)this.ReadInt24();

        public override uint ReadUInt32()
        {
            var buffer = this.ReadBytesWithEndian(4);
            return BitConverter.ToUInt32(buffer, 0);
        }

        public override ulong ReadUInt64()
        {
            var buffer = this.ReadBytesWithEndian(8);
