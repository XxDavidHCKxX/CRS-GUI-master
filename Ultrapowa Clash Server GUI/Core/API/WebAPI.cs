namespace CRS.Core.API
{
    ...
    internal class WebAPI
    {
        ...
        /// <summary>
        /// Initialize a new instance of the <see cref="WebAPI"/> class.
        /// </summary>
        public WebAPI()
        {
            this._Thread = new Thread(() =>
            {
                ...
                if (!this.Used)
                {
                    this.Listener.Prefixes.Add("http://" + Constants.ServerAddr + ":" + Constants.ServerWebPort + "/");
                    this.Listener.Start();

                    Console.WriteLine("The Web API has been sucessfully started on http://" + Constants.ServerAddr + ":" + Constants.ServerWebPort + "/");
        ...
        /// <summary>
        /// Processes the specified parameter.
        /// </summary>
        ...
        public void Process(object _Param)
        {
            ...
            try
            {
                if (Constants.AuthorizedIP.Contains(_Context.Request.RemoteEndPoint?.Address.ToString()))
                {
                ...
                {
                    this.Send(_Context, "unauthorized");
                }
            }
            catch (Exception)
            {
        ...
        public bool Used
        {
            get
            {
                ...
                return _Connections.Any(_Connection => _Connection.LocalEndPoint.Port == Constants.ServerWebPort);
            }
        }
    }
}