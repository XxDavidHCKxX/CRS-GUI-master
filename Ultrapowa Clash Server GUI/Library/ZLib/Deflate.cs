    ...
    internal sealed class DeflateManager
    {
        ...
        internal void pqdownheap(short[] tree, int k)
        {
            ...
            while (j <= this.heap_len)
            {
                ...
                if ((j < this.heap_len) && _IsSmaller(tree, this.heap[j + 1], this.heap[j], this.depth))
                {
                    j++;
                }
        ...
        internal BlockState DeflateSlow(FlushType flush)
        {
            ...
            while (true)
            {
                ...
                if ((hash_head != 0) && (this.prev_length < this.config.MaxLazy) && (((this.strstart - hash_head) & 0xffff) <= this.w_size - MIN_LOOKAHEAD))
                {
                    ...
                    // longest_match() sets match_start
                    if ((this.match_length <= 5) && ((this.compressionStrategy == CompressionStrategy.Filtered) || ((this.match_length == MIN_MATCH) && (this.strstart - this.match_start > 4096))))
                    {
