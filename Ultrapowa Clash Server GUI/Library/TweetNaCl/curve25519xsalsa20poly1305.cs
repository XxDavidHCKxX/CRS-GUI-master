namespace CRS.Library.TweetNaCl
{
    #region Usings
    ...
    public class curve25519xsalsa20poly1305
    {
        ...
        public static int crypto_box(byte[] c, byte[] m, byte[] n, byte[] pk, byte[] sk)
        {
            ...
            return crypto_box(cp, mp, m.Length, np, pkp, skp);
        }
        ...
        public static int crypto_box_afternm(byte[] c, byte[] m, byte[] n, byte[] k)
        {
            ...
            return crypto_box_afternm(cp, mp, m.Length, np, kp);
        }
        ...
        {
            byte[] cp = c, mp = m, np = n, pkp = pk, skp = sk;
            return crypto_box_open(mp, cp, c.Length, np, pkp, skp);
        }
        ...
        {
            byte[] cp = c, mp = m, np = n, kp = k;
            return crypto_box_open_afternm(mp, cp, c.Length, np, kp);
        }
    }
}